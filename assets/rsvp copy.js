...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!inv) {
    // 👇 no hay código → vuelve al inicio
...
function validarAcceso() {
  ...
  const invitado = invitados.find(
    inv => (inv.codigo || "").toLowerCase() === codigo.toLowerCase()
  );
  ...
  // Cargar datos directo
  mostrarDatosInvitado(invitado);
  generarOpcionesConfirmacion(invitado.cupos);
}
